...
import { parseYoutubeId } from "@/lib/recording-id";
...
export function YoutubePreview({ url, startSec = 0 }: Props) {
  ...
  const src =
    videoId && `https://www.youtube.com/watch?v=${videoId}${start > 0 ? `&start=${start}` : ""}`;
  ...
  return (
    <div
      ref={hostRef}
      className="aspect-video w-full overflow-hidden rounded-xl border border-border bg-black [&_iframe]:!h-full [&_iframe]:!w-full [&_youtube-video]:block [&_youtube-video]:h-full [&_youtube-video]:w-full"
    />
  );
}
